...
public class MasterOpponent implements Opponent {
  ...
  public int[] play(Player player) {
    ...
    //For round 4 and onwards, the Master AI will alternate between the average and top strategy. If the round is an even number, use average. Otherwise use top.
    if (count > 3) {
      if ((count % 2) == 0) {
        count++;
